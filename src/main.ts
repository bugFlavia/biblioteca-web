...
function carregar(): Biblioteca {
  ...
  if (salvo) return Biblioteca.fromJSON(salvo); // RNF06: recupera sem perda
...
const hoje = () => ($("[data-cy=hoje]") as HTMLInputElement).value;

// Painel que mostra QUAL requisito está sendo executado agora.
function registrar(rf: string, descricao: string): void {
...
function mensagem(texto: string, tipo: "ok" | "erro"): void {
...
function renderUsuarios(): void {
  ($("[data-cy=select-usuario]") as unknown as HTMLSelectElement).innerHTML =
    `<option value="u1">Ana Souza</option><option value="u2">Bruno Lima</option>`;
}

function renderResultados(livros: ReturnType<Biblioteca["pesquisar"]>): void {
  $("[data-cy=resultados]").innerHTML = livros
    .map(
      (l) => `<li data-cy="resultado" data-codigo="${l.codigo}">
...
function renderMeusEmprestimos(): void {
  ...
  $("[data-cy=meus-emprestimos]").innerHTML =
    ativos
      .map(
        (e) => `<li data-cy="meu-emprestimo" data-id="${e.id}">
...
function wire(): void {
  ...
  $("[data-cy=btn-login]").addEventListener("click", () => {
    const id = ($("[data-cy=select-usuario]") as unknown as HTMLSelectElement).value;
  ...
  $("[data-cy=btn-buscar]").addEventListener("click", () => {
    const termo = ($("[data-cy=busca]") as HTMLInputElement).value;
  ...
  $("[data-cy=resultados]").addEventListener("click", (ev) => {
    ...
    if (t.dataset.cy !== "btn-emprestar") return;
    try {
      ...
      if (err instanceof NaoAutenticado) {
        ...
        mensagem((err as Error).message, "erro");
    ...
    salvar();
    renderResultados(bib.pesquisar(($("[data-cy=busca]") as HTMLInputElement).value));
    renderMeusEmprestimos();
  });
  ...
  $("[data-cy=meus-emprestimos]").addEventListener("click", (ev) => {
    const t = ev.target as HTMLElement;
    if (t.dataset.cy !== "btn-devolver") return;
